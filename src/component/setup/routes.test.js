...
test('Validation - Path', (assert) => {
  ...
  assertValidationError(assert, definition, 'The path key must be present in the route object')
})

test('Validation - Path value', (assert) => {
  const definition = {
    foo: {
      path: 1,
    },
...
test('Validation - Params object', (assert) => {
  ...
  assertValidationError(
    assert,
    definition,
    'At path: params -- Expected an object, but received: 1'
  )
})
...
test('Validation - Params values', (assert) => {
  ...
  assertValidationError(
    assert,
    definition,
    'At path: params.baz -- Expected a string, but received: 1'
  )
})
...
test('Validation - Options object', (assert) => {
  ...
  assertValidationError(
    assert,
    definition,
    'At path: options -- Expected an object, but received: 1'
  )
})
...
test('Validation - Options default params: inHistory', (assert) => {
  ...
  assertValidationError(
    assert,
    definition,
    'At path: options.inHistory -- Expected a value of type boolean, but received: 1'
  )
})
...
test('Validation - Options default params: keepAlive', (assert) => {
  ...
  assertValidationError(
    assert,
    definition,
    'At path: options.keepAlive -- Expected a value of type boolean, but received: 1'
  )
})
...
test('Validation - Options params', (assert) => {
  ...
  assert.doesNotThrow(
    () => routes({}, definition),
    "Should not throw given extra options' properties"
  )
  assert.end()
})
...
test('Validation - Component function', (assert) => {
  ...
  assertValidationError(
    assert,
    definition,
    'At path: component -- Expected a function, but received: 1'
  )
})
...
test('Validation - Hooks event: before', (assert) => {
  ...
  assertValidationError(
    assert,
    definition,
    'At path: hooks.before -- Expected a function, but received: 1'
  )
})
