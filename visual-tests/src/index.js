...
async function dockerCiMode() {
  ...
  const childProc = $({ stdio: 'inherit' })`${runtime} run --network host \
    ...
    /bin/bash -c ${`npm install && RUNTIME_ENV=ci npm test:visual ${commandLineStr}`}
  `
  await childProc
...
async function compareCaptureMode() {
  ...
  const exampleBuildRes = await execa('npm', ['build:examples'], {
    stdio: stdioOption,
  })
  ...
  const serveExamplesChildProc = $({
    ...
    detached: true,
    cleanup: false,
  })`npm serve-examples --port ${argv.port}`
...
async function runTest(browserType = 'chromium') {
  ...
  await page.exposeFunction('snapshot', async (test, options) => {
    console.log('snapshot ::', test, option)
    snapshotsTested++
